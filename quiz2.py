...
class QuizAgent:
    ...
    def generate_quiz(self, topic: str, level: str, num_questions: int) -> List[Dict]:
        ...
        raw_questions = response.text.split("Q")[
            1:
        ]  # Split by 'Q' to extract questions
        ...
        for q in raw_questions:
            try:
                ...
                # Detect if a code snippet is present
                code_snippet = ""
                if "```" in parts[1]:
                    code_start = parts.index("```")
                    code_end = parts.index("```", code_start + 1)
                    code_snippet = "\n".join(
                        parts[code_start + 1 : code_end]
                    )  # Extract code
                    del parts[code_start : code_end + 1]  # Remove from list
                ...
                questions.append(
                    {
                        ...
                        "code_snippet": code_snippet,  # Store code separately
                        "options": options,
                        "correct": correct,
    ...
    def run_quiz(self, topic: str, level: str, num_questions: int):
        ...
        for i, q in enumerate(questions, 1):
            ...
            if q["code_snippet"]:
                print(Fore.YELLOW + "\nCode Snippet:\n" + Style.RESET_ALL)
                print(Fore.CYAN + q["code_snippet"] + Style.RESET_ALL)
